...
class ConfigurationClassEnhancer {
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		//主要是为了生成的CGLIB类中添加变量$$beanFactory
		//同时基于接口EnhancerConfiguration的父接口BeanFactoryAware中的setBeanFactory方法
		//设置此变量的值为当前Context中的beanFactory，这样一来我们这个cglib代理的对象就有了beanFactory
		//有了factory就能获得对象，而不用去通过方法获得对象了，因为通过方法获得对象不能控制器过程
		//该beanFactory的作用是在this调用是拦截该调用，并直接在beanFactory中获得目标bean
	...
	private static class ConditionalCallbackFilter implements CallbackFilter {
		...
		public int accept(Method method) {
			//过滤器会在这个地方遍历整个callbacks集合，调用callback的isMatch方法来确定
			//用哪个回调处理
	...
	private static class BeanFactoryAwareMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		//方法拦截器，由isMatch判断知道这个地方拦截的setBeanFactory
		//谁要执行到这个方法肯定是能传入一个beanFactory对象的
		//然后把这个对象塞到代理的class的$$beanFactory字段去
		@Override
		@Nullable
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//用过enhancedConfigInstance中cglib生成的成员变量$$beanFactory获得beanFactory
