...
class ConfigurationClassParser {
	...
	public void parse(Set<BeanDefinitionHolder> configCandidates) {
		for (BeanDefinitionHolder holder : configCandidates) {
			//根据BeanDefinition的类型做不同的处理，一般会调用ConfigurationClassParse#parse进行解析
			BeanDefinition bd = holder.getBeanDefinition();
			//
			try {
				if (bd instanceof AnnotatedBeanDefinition) {
					//解析注解对象，并且把解析出来的bd放到map中，但是这里的bd指的是普通的
					//何为不普通的呢？比如@Bean 和各种beanFactoryPostProcessor的得到的bean不再这里put
					//但是是这里解析，只是不put而已
	...
	protected void processConfigurationClass(ConfigurationClass configClass) throws IOException {
		...
		//把扫描出来的configClass存起来在后面会去处理这些类中的@Bean等注解
	...
	protected final SourceClass doProcessConfigurationClass(ConfigurationClass configClass, SourceClass sourceClass)
		...
		if (!componentScans.isEmpty() &&
			...
			for (AnnotationAttributes componentScan : componentScans) {
				...
				//扫描普通类==componentScan
				//这里扫描出来所有的@Component
				//并且把扫描出来的普通beanDefinition放到map中
				//@ComponentScan扫描出来的类会直接注册到容器中
				...
				for (BeanDefinitionHolder holder : scannedBeanDefinitions) {
					...
					if (ConfigurationClassUtils.checkConfigurationClassCandidate(bdCand, this.metadataReaderFactory)) {
						//这个地方有个递归，会先递归去注册扫描出来的beanDefinition中的扩展出来的bean注册到容器中
		...
		//这个里面会递归调用processConfigurationClass处理@Import注解
		//这里处理逻辑非常巧妙使用了一个双端队列ImportStack extends ArrayDeque（push是在队头加入对象，add是在队尾加入对象）
		//来解决嵌套的@Import
		//假如有嵌套@Import情况
		//A import B，B import C
		//程序处理配置类A的时候processImports执行到这里首先pushA到队列头，处理A的时候
		// 发现A又import B，然后转而递归去处理B B也会走到这一行代码processImports，先把自己push到队列头（B->A）,然后处理B import的
		//C对象，处理C对象，处理完C对象之后回到B对象并pop出队列头（剩余A），然后再处理完A对象结束。
		//这个地方处理不会把@Import的类放入到容器中（beanDefinitionMap），由于递归所以会放入到configurationClasses这个map中去
	...
	//会把数据放到ConfigurationClass中的Map<ImportBeanDefinitionRegistrar, AnnotationMetadata>
	//importBeanDefinitionRegistrars中去
	private void processImports(ConfigurationClass configClass, SourceClass currentSourceClass,
		...
		if (checkForCircularImports && isChainedImportOnStack(configClass)) {
			...
			//这个地方为什么要用双端队列吧这个configClass存起来？？？
			this.importStack.push(configClass);
			try {
				for (SourceClass candidate : importCandidates) {
					if (candidate.isAssignable(ImportSelector.class)) {
						...
						// ，是一个递归调用，因为Import的普通类，也有可能被加了Import注解，@ComponentScan注解 或者其他注解，
						// 所以普通类需要再次被解析
						processConfigurationClass(candidate.asConfigClass(configClass));
					}
				}
