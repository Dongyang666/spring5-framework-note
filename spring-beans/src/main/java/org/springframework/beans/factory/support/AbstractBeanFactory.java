...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		/**
		 *
		 * 这个方法是个空壳方法调用的是getSingleton(String beanName, boolean allowEarlyReference)
		 * 并且allowEarlyReference这个属性传入的是true
		 * 会从三个map中去尝试获取bean
		 * singletonObjects（一级缓存，单例池,存储的都是经过了整个spring周期的bean）
		 * earlySingletonObjects（三级缓存，把从singletonFactories中获取的bean缓存一下，非常重要涉及循环依赖
		 * ，这个里面存储的是经过了AOP过程的bean，不过还没走aop后面的流程）
		 * singletonFactories（二级缓存，里面存的一个lambada表达式（这个表达式会生成判断并生成AOP对象））
		 * 首先会从一级缓存获取如果获取不到，会从三级缓存里面拿一次拿不到，
		 * 并且allowEarlyReference为true才从二级缓存里面拿一下。
		 *
		 */
		...
		/**
		 *
		 * 从缓存里面出来的bean都需要走这一步，检测bean的正确性
		 * 检查当前bean是不是一个FactoryBean，如果是需要调用这个bean的getObject方法返回
		 */
	...
	protected Object getObjectForBeanInstance(
		...
		//如果当前的beanName是以&符号开头的，则证明是factoryBean
		//以&开头证明用户是想要FactoryBean对象而不是getObject的实例
		if (BeanFactoryUtils.isFactoryDereference(name)) {
		...
		//到这个地方一定是FactoryBean对象并且需要的对象是它的getObject方法的返回值
		Object object = null;
		if (mbd != null) {
			mbd.isFactoryBean = true;
		}
